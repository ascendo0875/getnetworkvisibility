...
class PartnersFeatured {
     ...
     */
    public static function getInstance($postId = false, $blockId = false): PartnersFeatured
    {
        if(!$postId && !$blockId) {
            return new self($postId, $blockId);
        }
        
        if((!isset(self::$instaces[$postId]) && !isset(self::$instaces[$postId][$blockId])) || !self::$instaces[$postId][$blockId] instanceof PartnersFeatured) {
    ...
    * @var string|false
    */
    private $labelForPartnerCta = null;
    ...
    */
     ...
     * @return string|false
     */
    public function getLabelForPartnerCta()
    {
        if((is_null($this->labelForPartnerCta) || (is_admin() && acf_is_block_editor()))) {
            $labelForPartnerCta = get_field('label_for_partner_cta');

            $this->setLabelForPartnerCta(!empty($labelForPartnerCta) ? $labelForPartnerCta : "see how we work with %s");
        }

        return $this->labelForPartnerCta;
    }

    /**
     * @param string|false $labelForPartnerCta
     * @return $this
     */
    public function setLabelForPartnerCta($labelForPartnerCta): self
    {
        $this->labelForPartnerCta = !empty($labelForPartnerCta) ? $labelForPartnerCta : false;
    ...
    public function getLabelCta()
    {
        if((is_null($this->labelCta) || (is_admin() && acf_is_block_editor()))) {
            ...
            $this->setLabelCta(!empty($labelCta) ? $labelCta : "See all security tech partners");
        }
