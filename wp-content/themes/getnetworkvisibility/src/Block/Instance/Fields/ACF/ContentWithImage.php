...
class ContentWithImage {
     ...
     */
    public static function getInstance($postId = false, $blockId = false): ContentWithImage
    {
        if(!$postId && !$blockId) {
            return new self($postId, $blockId);
        }
        
        if((!isset(self::$instaces[$postId]) && !isset(self::$instaces[$postId][$blockId])) || !self::$instaces[$postId][$blockId] instanceof ContentWithImage) {
