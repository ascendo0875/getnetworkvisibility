...
class MoreSolutionMarket {

    const DATAPOST_MARKET_LABEL = "Market";
    const DATAPOST_MARKET_VALUE = "market";
     ...
     * @return MoreSolutionMarket
     */
    public static function getInstance($postId = false, $blockId = false): MoreSolutionMarket
    {
        if(!$postId && !$blockId) {
            return new self($postId, $blockId);
        }
        
        if((!isset(self::$instaces[$postId]) && !isset(self::$instaces[$postId][$blockId])) || !self::$instaces[$postId][$blockId] instanceof MoreSolutionMarket) {
    ...
    */
    ...
    public function isMarket() {
        if((is_null($this->isMarket)  || (is_admin() && acf_is_block_editor()))) {
            $this->setMarket(in_array(self::DATAPOST_MARKET_VALUE, $this->getDataPost()));
        }
    ...
    {
        $this->isMarket = !empty($market) ? $market : false;
    ...
    {
        if((is_null($this->markets) || (is_admin() && acf_is_block_editor()))) {
            $markets = get_field('markets');
            
            $this->setMarkets(!empty($markets) ? $markets : null);
        }
    ...
    /**
     * @param array|false $markets
     * @return $this
     */
    public function setMarkets($markets): self
    {
